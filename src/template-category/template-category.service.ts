...
import { TemplateCategoryCopyRequestDto } from './dto/template-category.request.dto';
...
export class TemplateCategoryService {
  ...
  async removeCategoryFromTemplate(
    ...
    const templateCategoriesCount: number =
      await this.templateCategoryRepository.countTemplateCategories(templateCategoryIds);
  ...
  async copyTemplate(
    userId: number,
    id: number,
    templateCategoryCopyRequestDto: TemplateCategoryCopyRequestDto,
  ): Promise<void> {
    //템플릿 접근 권한 체크
    const templateAccess: TemplateEntity = await this.templateRepository.checkTemplateAccess(
      userId,
    ...
    const todos: Array<TodoRequestDto> = await categories.map((item) => {
      return {
        ...
        status: 'TODO',
        memo: null,
        today: templateCategoryCopyRequestDto['today'],
      };
    });
